...
static  unsigned long PackElementIntoDestinationImageFormat(unsigned
long Element, GRAPHICS_ELEMENT_FORMAT *pFormat)
{
        return  (((Element>>(8-pFormat->BPP))<<pFormat->Shift) & pFormat->Mask);
}

static  unsigned long UnPackElementFromSourceImageFormat(unsigned
...
static  void WritePackedImageData(unsigned char *pDest, unsigned
...
static  unsigned long ReadPackedImageData(unsigned char *pSource,
int NoOfBytes)
{
...
static  unsigned long PackRGBIntoDestinationImageFormat(unsigned
...
static  void     GetRGBFromSourceImageFormat(unsigned long
...
static  void CalcShiftAndBPPFromMask(unsigned long Mask, unsigned
...
static  void     BuildFormatInfoFromColourMasks(unsigned long
RedMask,
